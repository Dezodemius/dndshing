...
from app.auth import mailru_client
from app.auth.models import OAuthAccount, User
from app.core.config import get_settings

AUTHORIZE_URL = "/api/v1/auth/oauth/mailru/authorize"
CALLBACK_URL = "/api/v1/auth/oauth/mailru/callback"

_PROVIDER_USER_ID = "987654321"
_PROFILE = mailru_client.MailRuProfile(
    provider_user_id=_PROVIDER_USER_ID, email="player@example.com", display_name="Игрок Mail.ru"
)


@pytest.fixture(autouse=True)
def _mailru_config(monkeypatch: pytest.MonkeyPatch) -> None:
    settings = get_settings()
    monkeypatch.setattr(settings, "mailru_client_id", "client-id")
    monkeypatch.setattr(settings, "mailru_client_secret", "client-secret")
    monkeypatch.setattr(
        settings, "mailru_redirect_uri", "http://testserver/api/v1/auth/oauth/mailru/callback"
    )


@pytest.fixture
def mock_provider(monkeypatch: pytest.MonkeyPatch) -> mailru_client.MailRuProfile:
    ...
    async def _fake_fetch_profile(access_token: str) -> mailru_client.MailRuProfile:
        assert access_token == "fake-access-token"
        return _PROFILE

    monkeypatch.setattr(mailru_client, "exchange_code", _fake_exchange_code)
    monkeypatch.setattr(mailru_client, "fetch_profile", _fake_fetch_profile)
    return _PROFILE
...
async def test_authorize_redirects_to_mailru_and_sets_state_cookie(client: AsyncClient) -> None:
    response = await client.get(AUTHORIZE_URL, follow_redirects=False)

    assert response.status_code == 302
    assert response.headers["location"].startswith("https://oauth.mail.ru/login?")
...
async def test_authorize_is_disabled_without_config(
    ...
    monkeypatch.setattr(get_settings(), "mailru_client_id", None)
...
async def test_callback_with_mismatched_state_is_rejected(
    client: AsyncClient, mock_provider: mailru_client.MailRuProfile
) -> None:
...
async def test_callback_creates_new_verified_user(
    client: AsyncClient, mock_provider: mailru_client.MailRuProfile, db_session
) -> None:
...
async def test_callback_links_existing_account_by_email(
    client: AsyncClient, mock_provider: mailru_client.MailRuProfile, db_session: AsyncSession
) -> None:
    # An account row that pre-dates this Mail.ru login — e.g. created via a
