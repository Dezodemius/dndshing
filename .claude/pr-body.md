...
- Секретов и дефолтных кредов в репозитории нет: `Settings.database_url` — без значения по умолчанию (нет `DATABASE_URL` → приложение падает на старте), в compose креды через `${VAR:?}` (нет пароля → `docker compose up` не стартует), в `.env.example` пустые значения и команда генерации пароля. Порты `5432`/`8000` публикуются только на `127.0.0.1`; хост-порт БД настраивается через `POSTGRES_HOST_PORT` (5432 часто занят чужим постгресом).
...
1. `docker compose up -d --build` → `db` становится healthy, `api` стартует; `curl http://127.0.0.1:8000/healthz` → `200 {"status":"ok"}`.
...
5. Проверки на секреты, каждая — что провал громкий:
   - `docker compose config` без `.env` → `required variable POSTGRES_USER is missing a value`, стек не поднимается;
   - `get_settings()` без `DATABASE_URL` → `ValidationError`, приложение не стартует;
   - вход старым дефолтом `dndshing/dndshing` в поднятую БД → `FATAL: password authentication failed`;
   - `netstat` → БД слушает только `127.0.0.1`, наружу не торчит.
...
- Никаких дефолтов у секретов, даже «удобных для локалки»: дефолт в `Settings` означает, что прод стартанёт с кредами из публичного репозитория, если переменная потеряется, — и сделает это молча. Поэтому отсутствие `DATABASE_URL` роняет старт, а не подставляет фолбэк; `TEST_DATABASE_URL` остаётся опциональным (в проде он не нужен) и проверяется фикстурой pytest.
