...
- `op` — одна из восьми операций: `set`, `bonus`, `armor_base`, `advantage`, `disadvantage`, `resistance`, `immunity`, `vulnerability`.
- `value` обязателен для `set | bonus | armor_base` и запрещён для остальных.
...
**Порядок разрешения одной числовой цели:** за базу берётся наибольшее из (собственное значение персонажа, каждый кандидат `armor_base` = `value` + модификатор Ловкости, зажатый `dex_cap`, каждое значение `set`) → сверху суммируются `bonus` (внутри одного `stack_group` остаётся один, с наибольшим `abs(value)`) → зажим по цели: `ability.*` в 1..30, `speed` ≥ 0, `hp_max` ≥ 1, `ac` ≥ 0.

Операции `override` в словаре нет намеренно. В первом наброске она была рядом с `set`, но отличались они только тем, кто из них побеждает, — два написания для «заменить базу» превращаются в подбрасывание монетки для того, кто пишет контент-пак. `set` заменяет базу у любой цели; `armor_base` — единственное написание под КД, потому что доспех — единственный случай, где замена обязана заново прибавить сверху зажатый модификатор характеристики.
