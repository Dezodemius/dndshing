...
- **Деплой:** Docker Compose (api, postgres, nginx/caddy), Cloudflare перед сервером, GitHub Actions: тесты → build → deploy на self-hosted runner (схема как в резюмирую.рф). Бэкап Postgres — pg_dump по крону + выгрузка (аналог backup-db скилла).
...
Риски масштабирования (фиксируем, не решаем): JSONB-контент без версионирования правок — при обновлении пака персонажи «переезжают» на новые описания молча; один сервер делит ресурсы с резюмирую.рф; отсутствие лога транзакций сделает будущую CRM-аналитику ретроактивно невозможной (лог начнётся с момента внедрения).
